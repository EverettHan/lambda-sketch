...
float radinv_fl(uint n, uint dim) 
{
  ...
  const uint p_p = prime_permTN2[int(dim)];
...
void main()
{
  ...
  vec2 xi = vec2(0.0); // random variable for microfacet distribution	
    ...
    worldPos = GetEffectTexture(positionsNormalMap, vTexCoord.xy).xyz;
    vec3 worldSpaceCameraPos = cb_PostEffectAuto.u_EyePosition;
    vec3 viewDir =  vNormalize(worldPos - worldSpaceCameraPos);

    vec3 viewNormal = normal;
    vec3 worldNormal = (cb_PostEffectAuto.u_ViewInverseMatrix * vec4(viewNormal, 0.0)).xyz;
    worldNormal = vNormalize(worldNormal);

    normal = worldNormal;
    viewVect = viewDir;
  #endif

  int index = atomicAdd(pixelIndex, 1);

  xi.x = radinv_fl(cb_PostEffectAuto.u_FrameIdx + index, 2);
  xi.y = radinv_fl(cb_PostEffectAuto.u_FrameIdx + index, 3);

  #ifdef SO_BRDF_BIAS
    xi.y = vMix(xi.y, 0.0, 0.8);// BRDF Bias 0.8 to reduce noise
  #endif
   
  vec4 H = ImportanceSampleWithPDF(xi, roughness, normal);
  vec3 reflVect = vReflect(viewVect, H.xyz);

  float RoN = vDot(reflVect, normal);
  if (RoN < 0.0) reflVect = reflVect - 2.0 * normal * RoN;

  FRAGCOLOR = vec4(reflVect, H.w);
}
