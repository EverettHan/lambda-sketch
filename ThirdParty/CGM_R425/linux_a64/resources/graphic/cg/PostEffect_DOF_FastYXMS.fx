...
float4 Effect_DOFY_PS(pixelInput IN) : COLOR 
{
  ...
  for(unsigned int curSample = 0 ; curSample < Auto_NbAASamples ; ++curSample)
  {
    ...
    float blurRadius = GetBlurRadius(screenCoordToModif.xy, curSample)*(u_BlurSize*Auto_Size.x);  

    screenCoordToModif.y -= blurRadius;  
    ...
    {
      float ratio = GetBlurRadius(screenCoord.xy, curSample)*(1.f+u_BlurQuality-abs((float)i))+0.01;  

      col += texRECT(Auto_EyeMap, screenCoordToModif)*ratio; 
      num += ratio;

      screenCoordToModif.y += deltaRadius;
    }  
