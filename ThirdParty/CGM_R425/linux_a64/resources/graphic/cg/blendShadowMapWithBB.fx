    ...
    ) 
{
  ...
  float3 WorldNormal = mul( iWorldIT, IN.Normal ).xyz;
...
float ComputeSelfShadow(
    float3    iLightPosition,
    float3    iPosition,
    float3    iNormal)
{
  float shadowCoef = 1.f;
  
  float3 Ln = normalize(iLightPosition - iPosition);
  ...
  {
      shadowCoef = 0.0;
      if (dotProduct <= PENOMBRA)  shadowCoef = (PENOMBRA-dotProduct)*INV_PENOMBRA;
  }
...
{
  float4 result = float4(i_shadowColor.xyz, 0.0);

  float4 texCoordLight = mul(i_shadowMatrix,float4(IN.WorldPosition, 1));
  float2 coord = texCoordLight.xy/texCoordLight.w;
  if (coord.x > 0.f && coord.x <1.f && coord.y > 0.f && coord.y <1.f && texCoordLight.z>0) 
  {
    result.a = ComputeSelfShadow(i_lightPosition, IN.WorldPosition, sign(iVf)*IN.WorldNormal);
    if (result.a != 1.f)
    {
      result.a =  max(1-GetSpotShadowCoeffBruteForce(i_shadowTexture, texCoordLight, i_shadowtextureSizeInv), result.a);
    }
  }
    ...
    ): COLOR 
{
  ...
  if (coord.x > 0.f && coord.x <1.f && coord.y > 0.f && coord.y <1.f && texCoordLight.z>0) 
  { 
    result.a = ComputeSelfShadow(i_lightPosition, IN.WorldPosition, IN.WorldNormal);
    if (result.a != 1.f)
    {
      result.a =  max(1-GetSpotShadowCoeffBruteForce(i_shadowTexture, texCoordLight, i_shadowtextureSizeInv), result.a);
    }
    ...
    ): COLOR 
{
  ...
  {  
    ...
    if (result.a != 1.f)
    {
      result.a =  max(1-GetSpotShadowCoeffPoisson(i_shadowTexture, texCoordLight, i_shadowtextureSizeInv, NUM_BLUR_24, INV_NUM_BLUR_24, filterBlurData24), result.a);
    }
  }
    ...
    ): COLOR 
{
  ...
  if (coord.x > 0.f && coord.x <1.f && coord.y > 0.f && coord.y <1.f && texCoordLight.z>0) 
  {
    ...
    if (result.a != 1.f)
    {
      ...
      result.a =  max(1-shadowCoeff*INV_NUM_BLUR_12, result.a);
    }
  }
...
technique Shadow_NV40
{
  pass p0 
  {
    ...
    DepthFunc = LEqual;
    DepthMask = true;        
    VertexProgram = compile vp40 ShadowVS( wvp, world, worldIT);
