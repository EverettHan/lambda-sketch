...
vertexOutputClip NormalAlphaMapHighVS( vertexInput IN,
                              ...
                              ) 
{
    ...
    /// Clipping
    //------------------------
    float4 WVposition = mul(iWordlView,position);	        
    for (int i=0; i<6; i++)
    {
...
technique NormalAlphaMap_GLSL
{
    ...
    {		
        ...
        DepthMask = true;
        DepthFunc = LEqual;
        VertexProgram   = compile glslv NormalAlphaMapLowVS(wvp, wordlView, overloadNormalPlane);
        FragmentProgram = compile glslf NormalAlphaMapLowPS();
    }
}
