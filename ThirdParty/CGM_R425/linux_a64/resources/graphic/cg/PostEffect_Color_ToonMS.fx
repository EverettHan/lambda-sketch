...
float4 Effect_Color_Toon_PS(pixelInput IN) : COLOR 
{
  ...
  {
	  float  ratio    = tex2DMSfetch(Auto_NormalMap_WithTransparency,  IN.texCoord.xy, curSample).w;  
    if (ratio != 1.f) continue;
    col.w += ratio;
	}
	col.w *= 1.f/Auto_NbAASamples;  	
...
technique Effect_Color_Toon_HSV_GLSL
{
    pass p0 
    {		
