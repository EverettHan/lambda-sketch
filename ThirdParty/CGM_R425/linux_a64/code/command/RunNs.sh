...
if [ $? != 0 ] && [ "`uname`" = "Linux" ]
# NFO - For Linux Browser Open from Compass case
then
  whence xdg-open >/dev/null 2>&1
  if [ $? == 0 ] 
  then
	xdg-open "$1" &
    exit 0	
...
then
 # Last test before exit, try a deseoerate open (compliant to darwin)
 whence open >/dev/null 2>&1
 if [ $? = 0 ]
 then
  open "$1" &
  exit 0
