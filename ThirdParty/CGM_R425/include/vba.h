...
typedef interface IVbaSecurityUser IVbaSecurityUser;
#endif  // __cplusplus

// From windef.h in the Windows 10 SDK. Copied here for hosts
// which are linking older versions of the SDK.
...
enum AMSI_RESULT : int; // declared in amsi.h, https://msdn.microsoft.com/en-us/library/windows/desktop/dn889584(v=vs.85).aspx
...
typedef struct {
    ...
    /* 
    Controls the behavior of the Vba IDE and runtime components on high dpi displays. Can be set
    as follows:

    On Windows 10 (or greater) systems, Vba can function at a level of dpi awareness that is independent
    of the host. On those systems, setting this value to DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2,
    will direct Vba to provide the best level of dpi support that it can. That is, if Vba is able to support
    multi-monitor and dynamic dpi, it will. Alternatively, if any other value is specified, Vba will limit
    its level of dpi support to that level.

    On legacy (pre-Windows 10) systems, the host should pass the value matching the host level of dpi 
    awareness since dpi support is per-process. Hosts that are not dpi aware should pass 
    DPI_AWARENESS_CONTEXT_UNAWARE.

    If (DPI_AWARENESS_CONTEXT)0 is passed, Vba will default to legacy behavior where we scale based on 
    system dpi regardless of the host level of dpi awareness. Vba will not set the thread dpi awareness 
    context on systems where per-window dpi is supported.
    */
...
#define VBAINITINFO_EnableDigitalSignaturesOuterStorage        0x00000040L
...
// For IVbaSiteUtils' ShowEnableMacroDialog
typedef struct {
	const WCHAR *wzFilePath;
	void *pvDigSigStore;
	BOOL fAllowAlwaysTrust;
	BOOL fCancelOnly;
	DWORD dwHelpID;
	const WCHAR *wzHelpFile;
	HWND hwndParent;
} VBAENABLEMACRODIALOGINFO;


// Parameters from vbaui to the host site utils for the call to VBADigSigSignDlg
typedef struct {
	void *pvDigSigStore;
	void *pvDoc;
	BOOL fStdSdkHost;
	...
	const WCHAR *wzHelpFile;
	HWND hwndParent;
	VBADIGSIGTYPE vbasigtype;
} VBASIGNMACRODIALOGINFO;


// Parameters from vbaui to the host site utils for the call to VBADigSigSignProject
typedef struct {
	void *pvDigSigStore;
	void *pvDoc;
	void *pvVBProj;
...
// Parameters from vbaui to the host site utils for the call to VBADigSigVerifyProject
typedef struct {
	void *pvDigSigStore;
	HWND hwndParent;
	void *pvMacro;
	...
	BOOL *pfLoadFromText;
	VBADIGSIGTYPE vbasigtype;
	DigSigVerifyResult *pdsvVerificationResult;
} VBAVERIFYPROJECTINFO;
...
// IVbaSite4 interface
//---------------
// {D5A36CAF-9ADF-4b4e-8DC3-B609A72BBFB7}
//DEFINE_GUID(IID_IVbaSite4, 0xd5a36caf, 0x9adf, 0x4b4e, 0x8d, 0xc3, 0xb6, 0x9, 0xa7, 0x2b, 0xbf, 0xb7);

#undef  INTERFACE
#define INTERFACE  IVbaSite4
DECLARE_INTERFACE_(IVbaSite4, IUnknown)
{
	BEGIN_INTERFACE
	// *** IUnknown methods ****
	STDMETHOD(QueryInterface)(THIS_ REFIID riid, LPVOID FAR* ppvObj) PURE;
	...
	// *** IVbaSite4 methods ****
	STDMETHOD(FindFileWithVersion)(THIS_ LPCOLESTR lpstrFileName, 
                                    BSTR *pbstrFullPath, 
                                    WORD wMajor, 
                                    WORD wMinor, 
                                    LCID lcid, 
                                    LPCOLESTR lpstrProjName, 
                                    PHOSTREQUEST pHostRequest) PURE;
};

//---------------
// IVbaSite5 interface
//---------------
// {E2D479EA-BE9F-4C02-850B-C204DC3BA7D7}
//DEFINE_GUID(IID_IVbaSite5, 0xe2d479ea, 0xbe9f, 0x4c02, 0x85, 0xb, 0xc2, 0x4, 0xdc, 0x3b, 0xa7, 0xd7);

#undef  INTERFACE
#define INTERFACE  IVbaSite5
DECLARE_INTERFACE_(IVbaSite5, IUnknown)
{
    BEGIN_INTERFACE
    // *** IUnknown methods ****
    STDMETHOD(QueryInterface)(THIS_ REFIID riid, LPVOID FAR* ppvObj) PURE;
    ...
    // *** IVbaSite5 methods ****
    STDMETHOD_(BOOL, IsCapabilityAvailable)(THIS_ VBACAPABILITYTYPE capabilityType) PURE;
};

#undef  INTERFACE
#define INTERFACE  IVbaSecurityUser
DECLARE_INTERFACE(IVbaSecurityUser)
{
	/*
	...
	*/
	STDMETHOD_(BOOL, FPutSign) (THIS_ DWORD_PTR dwDocId, void *pvSign, DWORD cbSign) PURE;
};


//------------------------
// IVbaSiteUtils interface
//------------------------
// {75E791F9-E188-4379-9E82-887F15BF93A1}
//DEFINE_GUID(IID_IVbaSiteUtils, 0x75e791f9, 0xe188, 0x4379, 0x9e, 0x82, 0x88, 0x7f, 0x15, 0xbf, 0x93, 0xa1);

#undef  INTERFACE
#define INTERFACE  IVbaSiteUtils
interface __declspec(uuid("75E791F9-E188-4379-9E82-887F15BF93A1")) DECLSPEC_NOVTABLE IVbaSiteUtils : public IUnknown
{
	BEGIN_INTERFACE
	// *** IUnknown methods ****
	STDMETHOD(QueryInterface)(THIS_ REFIID riid, LPVOID FAR* ppvObj) PURE;
	...
	// *** IVbaSiteUtils methods ****
	STDMETHOD(GetFilePath)(THIS_ BSTR bstrFileName,
	                             LCID lcid,
	                             BSTR *bstrFilePath) PURE;
	STDMETHOD(RecordDebugTag)(THIS_ DWORD dwTag) PURE;
	STDMETHOD(GetHelpWindow)(THIS_ HWND *phwnd) PURE;
	STDMETHOD(ShowEnableMacroDialog)(THIS_ VBAENABLEMACRODIALOGINFO *pvemdInfo,
	                                       MacroLoadAction *pactionResult,
	                                       BOOL *pfAlwaysTrust) PURE;
	STDMETHOD(ShowVirusAlert)(THIS) PURE;

	// Set the interface for calling back into VBA to set the digital signature once it has been generated.
	// Hosts can opt to use this mechanism if they implement their own custom signing logic
	// as long as they provide VBA with an IVbaProjectSite implementation at initialization
	// that implements the PutSignature method
	STDMETHOD(SetVbaSecurityUser)(THIS_ IVbaSecurityUser *pVbaSecUser) PURE;

	// If client returns TRUE for this method, it is expected they implement the entire
	// site utils signing pipe, including VBADigSigSignDlg, VBADigSigSignProject, and VBADigSigVerifyProject.
	// With this set, GetDigSig and SetDigSig are never called because it is expected that these operations
	// are handled internal to the host.
	// Any VBA client who passes the dwcookie paramter to VbaInitialize with a value of 0x0FF1CE must also
	// return TRUE for ImplementsCustomSigning and implement the digital signature stack.
	STDMETHOD_(BOOL, ImplementsCustomSigning)(THIS) PURE;

	STDMETHOD_(SignDlgResult, VBADigSigSignDlg)(THIS_ VBASIGNMACRODIALOGINFO *pvsmdInfo) PURE;
	STDMETHOD_(BOOL, VBADigSigSignProject)(THIS_ VBASIGNPROJECTINFO *pvspInfo) PURE;
	STDMETHOD_(MacroLoadAction, VBADigSigVerifyProject)(THIS_ VBAVERIFYPROJECTINFO *pvvpInfo) PURE;
};
...
//----------------------
// IVbaProject2 interface
//----------------------
// {7598B9AB-D645-4999-B2C9-7B7003C6D3B3}
//DEFINE_GUID(IID_IVbaProject2, 0x7598b9ab, 0xd645, 0x4999, 0xb2, 0xc9, 0x7b, 0x70, 0x3, 0xc6, 0xd3, 0xb3);

#undef  INTERFACE
#define INTERFACE  IVbaProject2
DECLARE_INTERFACE_(IVbaProject2, IUnknown)
{
    BEGIN_INTERFACE
    // *** IUnknown methods ****
    STDMETHOD(QueryInterface)(THIS_ REFIID riid, LPVOID FAR* ppvObj) PURE;
    STDMETHOD_(ULONG, AddRef)(THIS) PURE;
    STDMETHOD_(ULONG, Release)(THIS) PURE;
    // *** IVbaProject2 methods ****
    STDMETHOD(SetRequireMalwareScan)(THIS_ BOOL fRequireMalwareScan) PURE;
};
...
//-----------------------------
// IVbaMessageRouting interface
//-----------------------------

// {A3D9792F-2C5D-4923-909D-6163BE4FF574}
//DEFINE_GUID(IID_IVbaMessageRouting, 0xa3d9792f, 0x2c5d, 0x4923, 0x90, 0x9d, 0x61, 0x63, 0xbe, 0x4f, 0xf5, 0x74);

#undef  INTERFACE
#define INTERFACE  IVbaMessageRouting
DECLARE_INTERFACE_(IVbaMessageRouting, IUnknown)
{
	BEGIN_INTERFACE
	// *** IUnknown methods ****
	STDMETHOD(QueryInterface)(THIS_ REFIID riid, LPVOID FAR* ppvObj) PURE;
	...
	// *** IVbaMessageRouting methods ***
	STDMETHOD_(void, PeekMessageHandler)(THIS_ MSG *pmsg) PURE;
	STDMETHOD_(BOOL, HandleMessageNeeded)(THIS_ UINT msg) PURE;
	STDMETHOD_(BOOL, PitbsFromHwndAndMessage)(THIS_ HWND hwnd, UINT msg, VOID **pvPitbs) PURE;
	STDMETHOD_(void, HandleMessage)(THIS_ VOID *pvPitbs, MSG *pmsg) PURE;
};
...
//-------------------------------------------------------------------------
// IVbaProjectSQMSite 
// 	This is a callback interface that allows Vba to pass raw data to the host for SQM logging
//-------------------------------------------------------------------------

// {D75400ED-8289-4b85-BB5A-3954E2D4EC39}
//DEFINE_GUID(IID_IVbaProjectSQMSite, 0xd75400ed, 0x8289, 0x4b85, 0xbb, 0x5a, 0x39, 0x54, 0xe2, 0xd4, 0xec, 0x39);

#undef  INTERFACE
#define INTERFACE  IVbaFiles
DECLARE_INTERFACE_(IVbaProjectSQMSite, IUnknown)
{
	BEGIN_INTERFACE
	// *** IUnknown methods ****
	STDMETHOD(QueryInterface)(THIS_ REFIID riid, LPVOID FAR* ppvObj) PURE;
	...
	STDMETHOD_(ULONG, Release)(THIS_) PURE;
  	...
  	STDMETHOD_(void, HostNotifyCompileDeclare)(THIS_ LPSTR szLibName, LPSTR szEntryPoint, VARIANT_BOOL fPtrSafe) PURE;
  	STDMETHOD_(void, HostNotifyCompileError)(THIS_ long lErr) PURE;
  	STDMETHOD_(void, HostNotifyCompileReferencedLibrary)(THIS_ GUID* pLibId) PURE;
  	STDMETHOD_(void, HostNotifyCompilePtrFunction) (THIS_) PURE;
 	STDMETHOD_(void, HostNotifyCompileLongLong )(THIS_) PURE;
	STDMETHOD_(void, HostNotifyCompileLongPtr) (THIS_) PURE;
        STDMETHOD_(void, HostNotifyComObjectInstantiated) (THIS_ REFCLSID clsid);
};

//-------------------------------------------------------------------------
// IVbaProjectSite2 
// This is a callback interface that allows Vba to pass raw data to the host
//-------------------------------------------------------------------------

// {E818A316-865F-43DD-8F55-A7A9E30A6F87}
//DEFINE_GUID(IID_IVbaProjectSite2, 0xE818A316, 0x865F, 0x43DD, 0x8F, 0x55, 0xA7, 0xA9, 0xE3, 0x0A, 0x6F, 0x87);

#undef  INTERFACE
#define INTERFACE  IVbaProjectSite2
DECLARE_INTERFACE_(IVbaProjectSite2, IUnknown)
{
    BEGIN_INTERFACE
    // *** IUnknown methods ****
    STDMETHOD(QueryInterface)(THIS_ REFIID riid, LPVOID FAR* ppvObj) PURE;
    ...
    STDMETHOD_(void, HostNotifyTelemetry)(THIS_ VBATELEMETRYDATAPOINT dp, VARIANT* pArgumentsArray) PURE;
    STDMETHOD_(void, HostNotifyMalwareFound)(THIS_ AMSI_RESULT amsiResult, VBAACTIONONMALWARE* pAction) PURE;
};
...
STDAPI VbaInitialize(DWORD_PTR dwCookie,
