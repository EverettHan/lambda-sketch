...
public:
	...
	BOOL PreTranslateMessage(MSG* pMsg)
	{
		...
		if (ApcHost.GetApcCompManager()) 
		{
			...
			ApcHost.GetApcCompManager()->APC_RAW(PreTranslateMessage)(&(pState->m_msgCur), &fConsumed);
...
/* Allow only VK_TAB and VK_DELETE for VBA PreTranslateMessage to avoid conflicts with the V5 message loop */
		...
		if (!ApcHost.m_fHostIsActiveComponent && pMsg) {
				...
				if (!::PeekMessage(&(pState->m_msgCur), NULL, NULL, NULL, PM_NOREMOVE)) 
				{
					if (bIdle) 
					{
						...
						if (!OnIdle(lIdleCount++)) 
						{
							...
							/* Begin DMO : Correction RI 0517222A */
							// Correction of a message loop synchronization problem which caused the V5 application to freeze
							// until a user intervention happens (like a mouse movement).  
							// This problem is precisely described at the following URL :
							// http://vba.summsoft.com/osc/vba6/KnowledgeBase/KB6mfc_msgloop.htm
							/***
							if (ApcHost.GetApcCompManager())
							...
							***/
							/* End DMO : Correction RI 0517222A */
						}
					}
					else 
					{
...
#define WM_KICKIDLE         0x036A  // (params unused) causes idles to kick in
	...
	virtual BOOL PumpMessage()
	{
		...
		if (fUnicodeWindowMsg)
		{
			...
			_AFX_THREAD_STATE* pState = AfxGetThreadState();
			ASSERT(pState);
			fReturn = ::GetMessageW(&(pState->m_msgCur), NULL, NULL, NULL);
		}
		else
		{
			...
			_AFX_THREAD_STATE* pState = AfxGetThreadState();
			ASSERT(pState);
			fReturn = ::GetMessageA(&(pState->m_msgCur), NULL, NULL, NULL);
		}
		...
		_AFX_THREAD_STATE* pState = AfxGetThreadState();
		ASSERT(pState);
		if ((pState->m_msgCur).message != WM_KICKIDLE && !PreTranslateMessage(&(pState->m_msgCur)))
		{
			::TranslateMessage(&(pState->m_msgCur));
			fUnicodeWindowMsg ? ::DispatchMessageW(&(pState->m_msgCur)) : ::DispatchMessageA(&(pState->m_msgCur));
		}
