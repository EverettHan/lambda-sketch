...
/*******************************************************************/
#ifndef CATICGMPolyhedralBodyDecimator_h_
#define CATICGMPolyhedralBodyDecimator_h_
...
extern ExportedByCATGMOperatorsInterfaces IID IID_CATICGMPolyhedralBodyDecimator;

/**
 * Operator for decimating a polyhedral CATBody.
 *
 * <b>Role:</b> This operator produces a polyhedral CATBody with a reduced
 *triangle count while maintaining a shape that is close to the shape of the
 *input.  This operator currently only works for polyhedral CATBody inputs.
 **/
class ExportedByCATGMOperatorsInterfaces CATICGMPolyhedralBodyDecimator
  : public CATICGMTopOperator
{
public:
  /**
   * Constructor
   */
  CATICGMPolyhedralBodyDecimator();
  ...
  /**
   * Sets a flag to prevent the collapse and change of any bar that is not
   * adjacent to two triangles (either on the same surface or across an edge.)
...
protected:
   ...
   * Destructor
   */
  virtual ~CATICGMPolyhedralBodyDecimator(); // -> delete can't be called
};

/**
 * Creates a CATICGMPolyhedralBodyDecimator operator.
 * @param iFactory
 * The factory of the geometry.
 * @param iTopData
 * The pointer to the data defining the software configuration and the journal.
 * If the journal inside <tt>iTopData</tt> is <tt>NULL</tt>, it is not filled.
 * @param iBody
 * The pointer to the body to decimate.
 * @return [out, IUnknown#Release]
 * The pointer to the created operator. To be released with the <tt>Release</tt>
 * method after use.
 */
ExportedByCATGMOperatorsInterfaces CATICGMPolyhedralBodyDecimator *
CATCGMCreatePolyhedralBodyDecimator(CATGeoFactory *iFactory,
                                    CATTopData *iTopData,
                                    CATBody *iBody);

#endif /* CATICGMPolyhedralBodyDecimator_h_ */
