...
//=============================================================================
//
// Maximal Distance Operator from one object to another object. The
// distance is given by the maximal distance found between the two
// objects, along the directions given by the normal vectors of the
// first operand/object. The acceleration structure is based on the
...
#ifndef CATPolyBVHMaximalDistance1To2_h
#define CATPolyBVHMaximalDistance1To2_h

#include "PolyBodyMeasureBetween.h"
#include "CATPolyBVHMeasureBetween.h"
#include "CATPolyBVBVOperator.h"

class BVHDistanceMax1To2Local_Distance;
 ...
 * generic Bounding Volume Hierarchies (BVH). These two hierarchies
 * should have been computed with the appropriate builder according to
 * the type of object given at the beginning (Visualization model,
 * NCGM model, etc.).
...
class ExportedByPolyBodyMeasureBetween CATPolyBVHMaximalDistance1To2 : public CATPolyBVHMeasureBetween
{
 public:

  CATPolyBVHMaximalDistance1To2(const CATTolerance & iTol = CATGetDefaultTolerance());

  virtual ~CATPolyBVHMaximalDistance1To2();
   ...
   *FALSE otherwise.
   */
  void SetFirstOperandInfinite(CATBoolean iInfiniteGeometry);
   ...
   *between the two objects).
   *@return S_OK if a distance has been computed (a distance measure
   *exists between the two given objects), S_FALSE otherwise.
   */
  virtual HRESULT GetResult(double & oMaximalDistance, CATMathPoint & oP1, CATMathPoint & oP2, DistanceType & oType) const;

  // /** deprecated
  //  */
  // CATBoolean GetResult(double & oMaximalDistance, CATMathPoint & oP1, CATMathPoint & oP2, DistanceType & oType);

  /** Gets the cells couple where is realized the minimum distance.
   *@param oCell1: the cell from the first argument where the
   *distance is realized.
   *@param oCell2: the cell from the second argument where the
   *distance is realized.
   */
 ...
 private:
  ...
  CATBoolean _FirstInfinite;

  BVHDistanceMax1To2Local_Distance * _MaximalDistance;
};

#endif //CATPolyBVHMaximalDistance1To2_h
