...
//===================================================================
// CATSGSYPArray.cpp
// Header definition of template class CATSGSYPArray and CATSGSYPStaticArray
//===================================================================
//
// Usage notes: 
//
...
*/ 
#ifndef CATSGSYPArray_H
#define CATSGSYPArray_H

#include <CATSGSYPMacros.h>
#include <CATSGSYPCheck.h>
#include <string.h>
#include <stdlib.h> // For qsort
#include <CATSGCoreToolsInline.h>

/** Possible actions to do when removing all elements of an array */
enum CATSGSYPArrayMemoryHandling
{
  /** releases the data. */
  CATSGSYPArrayReleaseAllocation = 0, 
  /** keeps the data allocated.*/
  CATSGSYPArrayKeepAllocation
};
 ...
 * If customer knows how much components he will need, he should pass it in the constructor ( #CATSGSYPConstArray(int i_capacity=0) )
 ...
 * In this case, it is very valuable to use the argument i_memoryHandling to CATSGSYPArrayKeepAllocation
 */
template<class T> class CATSGSYPConstArray
{
public:
  /**
   ...
   */
  INLINE explicit CATSGSYPConstArray(int i_capacity=0):_defaultValue(),_length(0),_plaArray(NULL), _capacity(0) {SetCapacity(i_capacity);}
  /**
   ...
   */
  INLINE CATSGSYPConstArray(const T* i_plrArray, int i_length):_defaultValue(),_length(0),_plaArray(NULL), _capacity(0) {if(i_plrArray){SetCapacity(i_length);AddArray(i_plrArray, i_length);}else{SG_CHECK_TRUE(i_length==0);}}
  /** Destructor */
  INLINE ~CATSGSYPConstArray(){delete [] _plaArray;_plaArray = NULL;}
  /**
   * Returns a const element at a given index, after checking that the index is inside the bounds of the array.
   */
  INLINE const T& Get(int i_index) const  { SG_CHECK_TRUE_RETV((i_index>=0 && i_index<_length), _defaultValue);return _plaArray[i_index];}
  /**
   * @copydoc CATSGSYPConstArray#Get
   */
   ...
   */
  INLINE int Get(int i_index, T& o_value) const {SG_CHECK_TRUE_RETV((i_index>=0 && i_index<_length), 0);o_value=_plaArray[i_index];return 1;}
  /**
   * Returns an element at a given index, after checking that the index is inside the bounds of the array.
   */
  INLINE T& Get(int i_index) {SG_CHECK_TRUE_RETV((i_index>=0 && i_index<_length), _defaultValue);return _plaArray[i_index];}
  /**
   * @copydoc CATSGSYPConstArray#Get
   */
   ...
   */
  INLINE void Set(int i_index, const T& i_value){SG_CHECK_TRUE_RET((i_index>=0 && i_index<_length));_plaArray[i_index]=i_value;}
  /**
  ...
  * @return a pointer on the inner array.
  ...
  {
    SG_CHECK_TRUE_RETV((i_index >=0 && i_index < _length), 0); 
  ...
  /**
   * Removes all values of the array. Warning, if i_memoryHandling=CATSGSYPArrayReleaseAllocation, no destructeur will be called!
   */
  INLINE void RemoveAll( CATSGSYPArrayMemoryHandling i_memoryHandling = CATSGSYPArrayReleaseAllocation)
  {
    _length=0;
    if (i_memoryHandling == CATSGSYPArrayReleaseAllocation)
    {
  ...
  INLINE void Swap(int i_index1, int i_index2 )
  {
    ...
    SG_CHECK_TRUE_RET(i_index1>=0 && i_index1<_length);
    SG_CHECK_TRUE_RET(i_index2>=0 && i_index2<_length);
  ...
  {
    SG_CHECK_TRUE_RET(i_index <= _length && i_index >= 0);
  ...
  /** Operator=*/
  INLINE CATSGSYPConstArray<T>& operator=(const CATSGSYPConstArray<T>& i_toCopy)
  {
  ...
  /** Copy constructor*/
  INLINE CATSGSYPConstArray(const CATSGSYPConstArray<T>& i_toCopy) : _plaArray(NULL)
  {
    _capacity = i_toCopy._capacity;
    _length = i_toCopy._length;
    if (_capacity > 0) _plaArray = new T[_capacity];
  ...
  INLINE void SetCapacity(int i_capacity, CATSGSYPArrayMemoryHandling i_memoryHandling = CATSGSYPArrayReleaseAllocation)
  {
    if (i_capacity==_capacity)return;
    if (i_memoryHandling == CATSGSYPArrayKeepAllocation && i_capacity < _capacity) return;
    SG_CHECK_TRUE_RET(i_capacity > _length);
    _capacity = i_capacity;
    T* plNewArray = new T[i_capacity];
    // Check that the allocation succeeded.
    SG_CHECK_POINTER_RET(plNewArray);
    if (_plaArray)
    {
  ...
  /** @deprecated */
  INLINE void QuickRemoveAll( CATSGSYPArrayMemoryHandling i_memoryHandling = CATSGSYPArrayReleaseAllocation){RemoveAll(i_memoryHandling);};
  /** @deprecated */
...
template<class T> class CATSGSYPArray : public CATSGSYPConstArray<T>
{
public:
  explicit CATSGSYPArray(int i_capacity=0): CATSGSYPConstArray<T>(i_capacity){}
  CATSGSYPArray(const T* i_plrArray, int i_length):CATSGSYPConstArray<T>(i_plrArray, i_length) {}
};

template<class T> class CATSGSYPDynArray : public CATSGSYPArray<T>
{
public:
  explicit CATSGSYPDynArray(int i_capacity=0): CATSGSYPArray<T>(i_capacity){}
  CATSGSYPDynArray(const T* i_plrArray, int i_length):CATSGSYPArray<T>(i_plrArray, i_length) {}
};
//-----------------------------------------------------------------------
