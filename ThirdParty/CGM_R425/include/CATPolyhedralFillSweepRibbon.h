#ifndef CATPolyhedralFillSweepRibbon_h_
#define CATPolyhedralFillSweepRibbon_h_

//=============================================================================
//
// COPYRIGHT : DASSAULT SYSTEMES 2018
//
// CATPolyhedralFillSweepRibbon
//
// DESCRIPTION :
//
// This operator builds a polyhedral ribbon shape along an input guide wire,
// filling the spaces between successive input sweep surfaces associated to some guide edges.
// If the guide is an open wire, it's mandatory to associate sweep surfaces to the first and last edges.
// Fill surfaces will be computed between successive sweep surfaces, sharing a guide vertex or separated by guide edges.
// The input guide can contain several domains but has to be manifold (no common vertices shared but several wires)
//
// History
//
// October 2018 DPS Creation
//
//=============================================================================
...
class ExportedByGMPolyShapeOper CATPolyhedralFillSweepRibbon : public CATGMPolyTopOperator
{
  CATCGMVirtualDeclareClass(CATPolyhedralFillSweepRibbon);
...
public:
   ...
   * Destructor
   */
  virtual ~CATPolyhedralFillSweepRibbon();
  ...
  * If iProfileBody is not correctly defined (null body, multi domains, nul vertex), 
  * or if this method is called after Run(), then it returns 0 without recording the data as inputs
  */
  ...
  * between 2 sweep bodies defined by their ID (see AddSweepBody / AddSweepProfile APIs)
  ...
  * @nodoc
  * @nocgmitf
  * Adds a specific filling body as the transition shape between 2 sweep bodies defined by their ID (see AddSweepBody API)
  *
  * @param iPrevSweepBodyID
  * The ID of the previous input body according to the guide wire orientation
  * @param iNextSweepBodyID
  * The ID of the next input body according to the guide wire orientation
  *
  * @param iFillingBody
  * The input body to use to extract the filling part between the 2 sweep bodies
  *
  * @return
  * The integer value as ID associated to the new transition, equals to the number of recorded transitions including this one.
  * If the input data are not correctly defined (null ID, null body), 
  * or if this method is called after Run(), then it returns 0 without recording this data as inputs
  */
  virtual int AddShapeTransition(int iPrevSweepBodyID, int iNextSweepBodyID, CATBody * iFillingBody) = 0;
  ...
  */
  virtual void EnableExtrapolationBeyondTarget(CATBody * iTargetBody) = 0;
	...
	*/
	virtual void GetOutputBoundaryEdgesOnGuide(CATLISTP(CATCell) &oBoundaryEdgesOnGuide, CATListOfInt &oEdgesOriVsGuide, CATSide *ioRibbonSideVsOrientedEdges = NULL) = 0;
	...
	* optional pointer to get the side (left or right) of the ribbon surface seen from the border edges according to their associated orientations
	*/
	virtual void GetOutputOppositeBoundaryEdges(CATLISTP(CATCell) &oOppositeBoundaryEdges, CATListOfInt &oEdgesOriVsGuide, CATSide *ioRibbonSideVsOrientedEdges = NULL) = 0;
  ...
  /**
  * Get the number of input SweepBodies given through the AddSweepBody / AddSweepProfileBody API
  ...
  */
  virtual int GetOutputSweepPatches(CATListOfInt &oSweepPatchesID, CATListOfInt * oInputSweepBodiesID = NULL) = 0;
...
protected:
  ...
  * @nodoc
  * @nocgmitf
  * Constructor cannot be called, constructor for derived classes only
  */
  CATPolyhedralFillSweepRibbon(CATGeoFactory & iFactory, const CATTopData & iTopData, CATGMPolyOperTopExt &iExtensible);
...
private:
  ...
  CATPolyhedralFillSweepRibbon();
  CATPolyhedralFillSweepRibbon(const CATPolyhedralFillSweepRibbon& iOneOf);
  CATPolyhedralFillSweepRibbon& operator =(const CATPolyhedralFillSweepRibbon& iOneOf);
};

/**
 * Creates a CATPolyhedralFillSweepRibbon operator with a guide wire body
 * @param iFactory
 ...
 */
ExportedByGMPolyShapeOper CATPolyhedralFillSweepRibbon * CATCreatePolyhedralFillSweepRibbon(CATGeoFactory *iFactory, CATTopData *iTopData, CATBody *iGuideWireBody);

#endif /* CATPolyhedralFillSweepRibbon_h_ */
