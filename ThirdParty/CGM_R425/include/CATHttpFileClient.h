...
//*===========================================================================*
#ifndef HTTPARCH
#ifndef CATHttpFileClient_H
#define CATHttpFileClient_H
#include "CATHttpAsyncClient.h"
#include "DSYSysStdIO.h"
#include "CATHttpClient.h"
#include "CATHttp.h"
#include "CATHttpDriver.h"




class CATSysHttpStreamer;
class ExportedByJS0HTTP CATHttpFileClient : public  CATHttpClient
{
protected:
	DSYSysFileDescriptor	_Filefd;
...
public:
	/* Construtor: Construtor to be used by default
	   ...
	   return: NONE
	   */
	CATHttpFileClient(const char *iUserAgent = NULL, short iMajor = 1, short iMinor = 1);
	/* DO NOT USE
		Consturctor only for licencing
	*/
	CATHttpFileClient(void* sslDriver);
	/*
		
	*/
	CATHttpFileClient( const char*iDriver, const char *iUserAgent, 
		short iMajor = 1, short iMinor = 1);
	~CATHttpFileClient();
    
	int Connect(const char *iHostName, unsigned short iPort = 80, 
		int iFlags = 0) override;
	int Connection(const char *iURL) override;
...
*/
	...
	int Get(const char *iWhatToGet, int iKeepConnection, int *oConnectionChanged = NULL) override;
	...
	int Head(const char *iInfo, CATFileInfo *info) override; //, int iFlags = 0) override;
	int Head64(const char *iInfo, CATFileInfo64 *info) override; //, int iFlags = 0) override;
...
*/
	...
	* @return
	*/
	int GetResponseHeaders(char **oHeader, size_t* oLen, size_t *oNext,
		DWORD iFlags = CATHTTP_QUERY_ALL, const char* iHeader = NULL) override;
