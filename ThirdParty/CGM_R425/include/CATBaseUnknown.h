...
#if !defined(_WINDOWS_SOURCE) && (__cplusplus < 201402L) && !defined(CATMetaClassStaticCheck_Disable)
/** @nodoc CATMetaClassStaticCheck requires C++14 support at least */
#define CATMetaClassStaticCheck_Disable
#endif
...
private:\
    ...
    template<typename SYS_OM_TEMPLATE_PARAM_T> /* template to prevent useless symbol export */              \
...
public:\
    template<typename SYS_OM_TEMPLATE_PARAM_T>                                                                  \
    static constexpr bool SYS_OM_META_CLASS_CHECK() {                                                           \
        /* Returns true if the method "SYS_RET_CLASS_TYPE" is defined in the class "SYS_OM_TEMPLATE_PARAM_T"  */\
        /* (in which case other CATMetaClass-related methods are expected to be defined too), false otherwise */\
        return std::is_same<SYS_OM_TEMPLATE_PARAM_T,                                                            \
            /* Extract the type of the first class which defines the "SYS_RET_CLASS_TYPE" method */             \
            /* in the inheritance tree of "SYS_OM_TEMPLATE_PARAM_T" (including itself)  */                      \
            typename std::remove_pointer<decltype(std::declval<SYS_OM_TEMPLATE_PARAM_T>().template SYS_RET_CLASS_TYPE<void>())>::type \
            >::value;\
    } 
#else
/** @nodoc */
