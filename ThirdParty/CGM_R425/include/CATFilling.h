...
// =============================================================================
// 
// RESPONSABLE  : P Mangin 
// 
// DESCRIPTION  : Class of Basic filling 
//     
// MPS  Oct 2019    : Ajout  de SetWithSameInnerMultiplicity
// MPS  Oct 2019    : Ajout  de ComputePlaneByFluxForFitting
// 
// KTU   Juil 2004   : Ajout  des  methodes  ComputeInitialSurface et SetMeshFunc
// 
// KTU : Mars 2004   : Ajout  ResetInitialSurface( .., .. ) = > for GDD
// MPS/KTU : 16/10/2003 : pour  LVL : Dans la methode SetNoTrimmedResult ajout de SetApproxParam(256,5 ) 
// 
// MPS : 03/08/2001  Pour QSR: possibilite de ne pas contraindre une edge 
// MPS : 04/04/2002   Pour QSR   possibilite de courbes de passage
//                      avec contraintes de tangence 
// KTU : le 13-05-02   : ajout de l'argument iWithFreeAnisotropie  avec valeur par defaut a la methode SetResolParam
//                     : cette var n'intervient que si iWithAnisotropie==FALSE et qu'on autorise 
//                     : tout de meme l'algo a calculer l'anisotropie et de l'utiliser si besoin
// MPS : Oct 2002 : Pour QSR : methode Add pour associer une normale a un point.
// MPS : Oct 2002 : Pour QSR : methode Add pour associer un (U,V ) a un point.
// MPS : Nov 2002 : Pour QSR : methode Add pour associer une courbe3d et une courbe representant
//                             la normale en tout point de la courbe.
// MPS : Janv 2003 : Pour QSR  dans la methode Add pour associer une normale a un point ajout
//                       de la tolerance G1
// MPS : Avril 2003 ajout tolerance G1 dans la methode Add (edge, face...) 
// MPS : Mai 2003 : ajout methode  
//                  int Add(CATEdge * iE, CATICContact * iContact, const CATBoolean iIsBound = 1,
//                          const double iTolerance = 0.0, const CATBoolean isFree=FALSE);
//                  afin de supprimer  la methode 
//                  int Add(CATEdge * Edge, CATEdge * EdgeNormal,
//                          const double ToleranceG0 = 0.0, const double ToleranceG1 = 0.5);
//      
// =============================================================================
//
...
class ExportedByCATSurfacicTopoOperators   CATFilling   : public   CATTopOperator
//class ExportedByCATSurfacicTopoOperators   CATFilling
{

  CATCGMVirtualDeclareClass(CATFilling);
public:
   //
   // Destructeur 
  virtual ~CATFilling();
  ...
  //  if IsFree=TRUE the Edge iE is not constraint 
   ...
   virtual int  Add(CATEdge * iE, CATFace * iF, const short iOrder ,const double iToleranceG0 = 0.0, const CATBoolean iIsBound = 1,
   ...
   virtual  int Add (ListPOfCATEdge & iListEdges,CATListOfInt & iListOfFree,CATListOfDouble & iListOfTol) = 0;
  ...
  //To Define an inner wire  with support
   ...
   // 
   // To Add N inner Wires  this method must be called N times 
   virtual int  Add (ListPOfCATEdge & iListEdges, ListPOfCATFace & iListFaces,CATListOfInt & iListOfOrder,CATListOfDouble & iListOfTol) = 0;
   ...
   virtual int  Add(const CATPoint * iP,CATMathVector &iNormal,const double ToleranceG0=0.0,CATBoolean HasContactG0=1,const double ToleranceG1=0.5) = 0;
   

   // To define a punctual G0 constraint and associate U,V parameters 
   virtual int  Add(const CATPoint * iPoint, const double U,const double V,const double Tolerance = 0.0) = 0;
   ...
   virtual int  Add(CATEdge * Edge, CATEdge * EdgeNormal,
                   const double ToleranceG0 = 0.0, const double ToleranceG1 = 0.5) = 0;
   ...
   //  if iTolerance == 0.0 the default tolerance (resolution) is used.
    //  if IsFree=TRUE the Edge iE is not constraint 
   virtual int  Add(CATEdge * iE, CATICContact * iContact, const CATBoolean iIsBound = 1, const double iTolerance = 0.0,
                   const CATBoolean isFree=FALSE) = 0;
  ...
  //  To reset Initial Surface = > for GDD
   virtual int  ResetInitialSurface(CATSurface * iSurfInit, const CATBoolean iDeformAll = 0) = 0;
    
   // ToDefine Tolerances G1 and G2
   virtual void SetTolerance ( const double TolAngular = 0.5, //in degree
                               const double TolCurvature  = 0.1 ) = 0; 

   // ToDefine algorithm paameter
   virtual int  SetResolParam(const short iOrder = 3, 
   ...
   virtual int  SetApproxParam (const int MaxNumberOfPatch = 64, const short Maxdegree = 8) = 0;
   virtual int  SetApproxParam (const int MaxUNumberOfPatch , const int MaxVNumberOfPatch ,
                               const short MaxUdegree ,const short MaxVdegree ) = 0;
 ...
 // to use  HigherExponant in PlateFE// gdd 
  ...
  // To set the Body to seew with the filling result
   ...
   // journal pour live shape clash
   /**  @nodoc @nocgmitf */ 
   virtual void SetModifJournalFix() = 0;
   ...
   virtual void SetDiscretizationByLength() = 0;
	//  SetRoundnessFactor : 
	/**  @nodoc @nocgmitf */
	virtual void SetRoundnessFactor(const double iValue) = 0;

	//  SetWithSameInnerMultiplicity : 
	/**  @nodoc @nocgmitf */
	virtual void  SetWithSameInnerMultiplicity() = 0;
...
// with null projection if isInitialSurfacePlane=2 otherwise it returns 0
   ...
   virtual int  Link (const CATEdge * iE,  CATPCurve * iPCurve,const CATCrvLimits & iLimits) = 0; 
...
// Get the initial surface 
   ...
   */
   ...
   virtual CATBoolean GetNextIncompatibility( int & oNumCont1,int & oNumCont2, 
                                              double & oParam1, double & oParam2, 
  ...
  CATFilling (CATGeoFactory* iFactory, CATTopData* iTopData);
...
CATFilling * CATCreateFilling(CATGeoFactory  *iFactory, CATTopData *iData);
#endif
